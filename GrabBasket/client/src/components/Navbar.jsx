...
const Navbar = () => {
  ...
  // Handle Contact click to scroll properly
  ...
  return (
    ...
    >
      ...
      <div className="hidden sm:flex items-center gap-8 font-medium">
        ...
        {/* Contact link with click handler */}
        <a
          href="/#newsletter"
          onClick={handleContactClick}
          className="relative group text-transition duration-300"
        >
          Contact
          <span className={`absolute left-0 -bottom-1 h-0.5 w-0 group-hover:w-full transition-all duration-300 ${isScrolled ? 'bg-black' : 'bg-white'}`}></span>
        </a>
        ...
        >
          ...
          <button className={`absolute -top-2 -right-3 text-xs w-[18px] h-[18px] rounded-full transition-all duration-300 ${isScrolled ? 'bg-[#002395] text-white' : 'bg-[#DBEAFE] text-black'}`}>
            {getCartCount()}
          </button>
        </div>
